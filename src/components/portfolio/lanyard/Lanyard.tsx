...
function Band({
  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current) {
      ...
      curve.points[1].copy(getLerped(j2.current as LanyardRigidBody));
      curve.points[2].copy(getLerped(j1.current as LanyardRigidBody));
  ...
  return (
    <>
      <group position={[0, 4, 0]}>
        ...
        >
          <CuboidCollider args={[0.95, 1.28, 0.01]} />
          <group
            scale={[2.65, 2.55, 2.25]}
