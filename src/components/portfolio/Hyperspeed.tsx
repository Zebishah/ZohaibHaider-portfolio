...
const distortions: Distortions = {
  mountainDistortion: {
    ...
    getDistortion: `uniform vec3 uAmp;uniform vec3 uFreq;#define PI 3.14159265358979
      ...
      vec3 getDistortion(float progress){float m=0.02;
        return vec3(cos(progress*PI*uFreq.x+uTime)*uAmp.x-cos(m*PI*uFreq.x+uTime)*uAmp.x,
  ...
  LongRaceDistortion: {
    ...
    getDistortion: `uniform vec2 uFreq;uniform vec2 uAmp;#define PI 3.14159265358979
      vec3 getDistortion(float progress){float c=0.0125;
        return vec3(sin(progress*PI*uFreq.x+uTime)*uAmp.x-sin(c*PI*uFreq.x+uTime)*uAmp.x,
...
class CarLights {
  ...
  init() {
    ...
    const colorArray = Array.isArray(this.colors)
      ...
      : [new THREE.Color(this.colors as any)];
...
class LightsSticks {
  ...
  init() {
    ...
    const colorArray = Array.isArray(options.colors.sticks)
      ? (options.colors.sticks as any[]).map((c) => new THREE.Color(c))
...
class App {
  ...
  constructor(container: HTMLElement, options: HyperspeedOptions) {
    ...
    if (!this.options.distortion) {
      this.options.distortion = { uniforms: {}, getDistortion: "vec3 getDistortion(float p){return vec3(0.);}" };
    }
  ...
  update(delta: number) {
    ...
    if (typeof this.options.distortion === "object" && this.options.distortion.getJS) {
      ...
      this.camera.lookAt(
        new THREE.Vector3(this.camera.position.x + distortion.x, this.camera.position.y + distortion.y, this.camera.position.z + distortion.z),
      );
      updateCamera = true;
    }
